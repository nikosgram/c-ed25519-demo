...
static int compressMessage(uint64_t *state, const unsigned char *buf) {
    ...
    const uint64_t d = 19 & 63, e = 61 & 63, f = 64 - d, g = 64 - e, h = 1 & 63, j = 8 & 63, k = 64 - h, l = 64 - j;

    for (i = 16; i < 80; i++) {
        const uint64_t a = w[i - 2], b = w[i - 15];

        w[i] = ((((a & UI64_MAX) >> d | a << f) & UI64_MAX) ^ (((a & UI64_MAX) >> e | a << g) & UI64_MAX) ^
                ((a & UI64_MAX) >> 6)) + w[i - 7] +
               ((((b & UI64_MAX) >> h | b << k) & UI64_MAX) ^ (((b & UI64_MAX) >> j | b << l) & UI64_MAX) ^
                ((b & UI64_MAX) >> 7)) + w[i - 16];
    }

    const uint64_t m = 14 & 63, n = 18 & 63, o = 41 & 63, p = 64 - m, q = 64 - n, r = 64 - o, t = 28 & 63, u =
            34 & 63, v = 39 & 63, x = 64 - t, y = 64 - u, z = 64 - v;

    for (i = 0; i < 80; i += 8) {
        t0 = s[7] +
             ((((s[4] & UI64_MAX) >> m | s[4] << p) & UI64_MAX) ^ (((s[4] & UI64_MAX) >> n | s[4] << q) & UI64_MAX) ^
              (((s[4] & UI64_MAX) >> o | s[4] << r) & UI64_MAX)) + (s[6] ^ (s[4] & (s[5] ^ s[6]))) + K[i] + w[i];
        s[3] += t0;
        s[7] = t0 +
               (((((s[0] & UI64_MAX) >> t | s[0] << x) & UI64_MAX) ^ (((s[0] & UI64_MAX) >> u | s[0] << y) & UI64_MAX) ^
                 (((s[0] & UI64_MAX) >> v | s[0] << z) & UI64_MAX)) + ((s[0] | s[1]) & s[2] | s[0] & s[1]));

        t0 = s[6] +
             ((((s[3] & UI64_MAX) >> m | s[3] << p) & UI64_MAX) ^ (((s[3] & UI64_MAX) >> n | s[3] << q) & UI64_MAX) ^
              (((s[3] & UI64_MAX) >> o | s[3] << r) & UI64_MAX)) + (s[5] ^ (s[3] & (s[4] ^ s[5]))) + K[i + 1] +
             w[i + 1];
        s[2] += t0;
        s[6] = t0 +
               (((((s[7] & UI64_MAX) >> t | s[7] << x) & UI64_MAX) ^ (((s[7] & UI64_MAX) >> u | s[7] << y) & UI64_MAX) ^
                 (((s[7] & UI64_MAX) >> v | s[7] << z) & UI64_MAX)) + ((s[7] | s[0]) & s[1] | s[7] & s[0]));

        t0 = s[5] +
             ((((s[2] & UI64_MAX) >> m | s[2] << p) & UI64_MAX) ^ (((s[2] & UI64_MAX) >> n | s[2] << q) & UI64_MAX) ^
              (((s[2] & UI64_MAX) >> o | s[2] << r) & UI64_MAX)) + (s[4] ^ (s[2] & (s[3] ^ s[4]))) + K[i + 2] +
             w[i + 2];
        s[1] += t0;
        s[5] = t0 +
               (((((s[6] & UI64_MAX) >> t | s[6] << x) & UI64_MAX) ^ (((s[6] & UI64_MAX) >> u | s[6] << y) & UI64_MAX) ^
                 (((s[6] & UI64_MAX) >> v | s[6] << z) & UI64_MAX)) + ((s[6] | s[7]) & s[0] | s[6] & s[7]));

        t0 = s[4] +
             ((((s[1] & UI64_MAX) >> m | s[1] << p) & UI64_MAX) ^ (((s[1] & UI64_MAX) >> n | s[1] << q) & UI64_MAX) ^
              (((s[1] & UI64_MAX) >> o | s[1] << r) & UI64_MAX)) + (s[3] ^ (s[1] & (s[2] ^ s[3]))) + K[i + 3] +
             w[i + 3];
        s[0] += t0;
        s[4] = t0 +
               (((((s[5] & UI64_MAX) >> t | s[5] << x) & UI64_MAX) ^ (((s[5] & UI64_MAX) >> u | s[5] << y) & UI64_MAX) ^
                 (((s[5] & UI64_MAX) >> v | s[5] << z) & UI64_MAX)) + ((s[5] | s[6]) & s[7] | s[5] & s[6]));

        t0 = s[3] +
             ((((s[0] & UI64_MAX) >> m | s[0] << p) & UI64_MAX) ^ (((s[0] & UI64_MAX) >> n | s[0] << q) & UI64_MAX) ^
              (((s[0] & UI64_MAX) >> o | s[0] << r) & UI64_MAX)) + (s[2] ^ (s[0] & (s[1] ^ s[2]))) + K[i + 4] +
             w[i + 4];
        s[7] += t0;
        s[3] = t0 +
               (((((s[4] & UI64_MAX) >> t | s[4] << x) & UI64_MAX) ^ (((s[4] & UI64_MAX) >> u | s[4] << y) & UI64_MAX) ^
                 (((s[4] & UI64_MAX) >> v | s[4] << z) & UI64_MAX)) + ((s[4] | s[5]) & s[6] | s[4] & s[5]));

        t0 = s[2] +
             ((((s[7] & UI64_MAX) >> m | s[7] << p) & UI64_MAX) ^ (((s[7] & UI64_MAX) >> n | s[7] << q) & UI64_MAX) ^
              (((s[7] & UI64_MAX) >> o | s[7] << r) & UI64_MAX)) + (s[1] ^ (s[7] & (s[0] ^ s[1]))) + K[i + 5] +
             w[i + 5];
        s[6] += t0;
        s[2] = t0 +
               (((((s[3] & UI64_MAX) >> t | s[3] << x) & UI64_MAX) ^ (((s[3] & UI64_MAX) >> u | s[3] << y) & UI64_MAX) ^
                 (((s[3] & UI64_MAX) >> v | s[3] << z) & UI64_MAX)) + ((s[3] | s[4]) & s[5] | s[3] & s[4]));

        t0 = s[1] +
             ((((s[6] & UI64_MAX) >> m | s[6] << p) & UI64_MAX) ^ (((s[6] & UI64_MAX) >> n | s[6] << q) & UI64_MAX) ^
              (((s[6] & UI64_MAX) >> o | s[6] << r) & UI64_MAX)) + (s[0] ^ (s[6] & (s[7] ^ s[0]))) + K[i + 6] +
             w[i + 6];
        s[5] += t0;
        s[1] = t0 +
               (((((s[2] & UI64_MAX) >> t | s[2] << x) & UI64_MAX) ^ (((s[2] & UI64_MAX) >> u | s[2] << y) & UI64_MAX) ^
                 (((s[2] & UI64_MAX) >> v | s[2] << z) & UI64_MAX)) + ((s[2] | s[3]) & s[4] | s[2] & s[3]));

        t0 = s[0] +
             ((((s[5] & UI64_MAX) >> m | s[5] << p) & UI64_MAX) ^ (((s[5] & UI64_MAX) >> n | s[5] << q) & UI64_MAX) ^
              (((s[5] & UI64_MAX) >> o | s[5] << r) & UI64_MAX)) + (s[7] ^ (s[5] & (s[6] ^ s[7]))) + K[i + 7] +
             w[i + 7];
        s[4] += t0;
        s[0] = t0 +
               (((((s[1] & UI64_MAX) >> t | s[1] << x) & UI64_MAX) ^ (((s[1] & UI64_MAX) >> u | s[1] << y) & UI64_MAX) ^
                 (((s[1] & UI64_MAX) >> v | s[1] << z) & UI64_MAX)) + ((s[1] | s[2]) & s[3] | s[1] & s[2]));
    }
